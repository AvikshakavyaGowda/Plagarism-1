...
var _nGram = require("@drorgl/n-gram");
...
var convertString = function convertString() {
  ...
  return "  ".concat(input.trim().replace(/\s+/g, ' ') // replace multiple spaces w/ single spaces
...
var trigramSimilarity = function trigramSimilarity() {
  var input1 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var input2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  if (input1 && input1.trim() && input1 === input2) return 1;
  var trigrams1 = generateTrigram(input1);
  var trigrams2 = generateTrigram(input2); // Total trigrams

  var total = (0, _toConsumableArray2["default"])(new Set([].concat((0, _toConsumableArray2["default"])(trigrams1), (0, _toConsumableArray2["default"])(trigrams2)))); // Trigrams both have in common

  var common = [];
  trigrams1.forEach(function (trigramItem) {
    if (trigrams2.includes(trigramItem)) common.push(trigramItem);
  });
  return total.length === 0 ? 0 : common.length / total.length;
};

var _default = trigramSimilarity;
